...
/** Colors the tools take where the gcode states none */
const DEFAULT_TOOL_COLORS = ['#0b2c7a', '#1c8891', '#aaf200', '#f5ce0a', '#d16830', '#942616']
...
export function resolveToolColors (statedColors: string[]): PropertyFixedColors {
  const tools = Math.max(statedColors.length, DEFAULT_TOOL_COLORS.length)

  return {
    colors: Array.from({ length: tools }, (_unused, tool) =>
      hexStringToLinearColor(statedColors[tool] || DEFAULT_TOOL_COLORS[tool % DEFAULT_TOOL_COLORS.length])),
    defaultColor: hexStringToLinearColor(DEFAULT_TOOL_COLORS[0]),
